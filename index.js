...
document.getElementById("btnTinhTienDien").onclick = function () {
  ...
  var tienDienHTML =
    "Tiền điện của khách hàng " + tenKhachHang + " là: " + giaDien + " đồng.";
...
document.getElementById("btnTinhTienThue").onclick = function () {
  ...
  tienThue = (thuNhapChiuThue * thueSuat) / 100;
  ...
  var ketQuaHTML =
    ...
    tienThue.toFixed(2) +
    " triệu đồng.";
  document.getElementById("TienThue").innerHTML = ketQuaHTML;
};
...
document.addEventListener("DOMContentLoaded", function () {
  document
    ...
    .addEventListener("change", function () {
      ...
      var soKetNoiLabel = document.querySelector("label[for='soKetNoi']");

      if (loaiKhachHang === "nhaDan") {
        soKetNoiInput.disabled = true;
        soKetNoiLabel.style.display = "none";
      } else {
        soKetNoiInput.disabled = false;
        soKetNoiLabel.style.display = "block";
      }
    });
  ...
  document
    ...
    .addEventListener("click", function () {
      ...
      var tienKenhCaoCap = 0;
      var tienCap = 0;

      if (loaiKhachHang === "nhaDan") {
        phiXuLyHoaDon = 4.5;
        phiDichVuCoBan = 20.5;
        tienKenhCaoCap = 7.5 * soKenhCaoCap;
      } else {
        phiXuLyHoaDon = 15;
        phiDichVuCoBan = 75 + (soKetNoi - 10) * 5;
        tienKenhCaoCap = 50 * soKenhCaoCap;
      }

      tienCap = phiXuLyHoaDon + phiDichVuCoBan + tienKenhCaoCap;

      var ketQuaHTML =
        "Tiền cáp của khách hàng " +
        maKhachHang +
        " là: $" +
        tienCap.toFixed(2);
      document.getElementById("tienCap").innerHTML = ketQuaHTML;
    });
});
